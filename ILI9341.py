...
class Display(object):
    ...
    def draw_bitmap_line(self, x, y, color_row):
            """Draw a horizontal line (row of pixels) on the screen."""
    ...
    def _set_window(self, x0, y0, x1, y1):
        """Set the window to the rectangle (x0, y0) to (x1, y1)."""
        # Column address set
        self._write_command(0x2A)
        self._write_data(x0.to_bytes(2, 'big'))
        self._write_data(x1.to_bytes(2, 'big'))
        
        # Page address set
        self._write_command(0x2B)
        self._write_data(y0.to_bytes(2, 'big'))
        self._write_data(y1.to_bytes(2, 'big'))
    ...
    def _write_command(self, command):
        ...
        self.cs(0)  # Select the display (active low)
        self.spi.write(bytearray([command]))  # Send the command
    ...
    def _write_data(self, data):
        ...
        if isinstance(data, int):
            # Convert integer to a byte and send it
            self.spi.write(bytearray([data]))
        else:
            # Assume data is already a byte array
            self.spi.write(data)
        self.cs(1)  # Deselect the display
        
    def rotate(self, rotation):
        """Rotate the screen to the specified orientation (0, 90, 180, 270 degrees)."""
        rotation_map = {
            0: 0x00,   # No rotation
            90: 0x60,  # 90 degrees (landscape)
            180: 0xC0, # 180 degrees (portrait upside down)
            270: 0xA0  # 270 degrees (landscape upside down)
        }
        
        if rotation not in rotation_map:
            raise ValueError("Invalid rotation value. Choose from 0, 90, 180, 270 degrees.")
        
        # Send the command to rotate the screen
        self._write_command(0x36)  # Memory Access Control command
        self._write_data(bytearray([rotation_map[rotation]]))

    def draw_bitmap_lines(self, x, y, bitmap, width):
        """Draw a bitmap line by line."""
        for row_index, row in enumerate(bitmap):
            for col in range(width):
                color = row[col]  # Get pixel color from the row
                self.draw_pixel(x + col, y + row_index, color)  # Draw pixel at the calculated position
